...
#         # feature projection
#         self.linear = nn.Conv2d(backbone.get_dimension(), dim, 1)
#         self.depth_proj = nn.Conv2d(1, dim, 1)  # 🔑 深度图映射到 dim
...
#                         img_head_maps.append(torch.zeros(self.featmap_h, self.featmap_w, device=self.linear.weight.device))
#                         continue
#                     xmin = torch.round(torch.tensor([xmin], device=self.linear.weight.device).float() * width).int()
#                     ymin = torch.round(torch.tensor([ymin], device=self.linear.weight.device).float() * height).int()
#                     xmax = torch.round(torch.tensor([xmax], device=self.linear.weight.device).float() * width).int()
#                     ymax = torch.round(torch.tensor([ymax], device=self.linear.weight.device).float() * height).int()
